...
const getVendor = async (req, res) => {
    const { id } = req.params;
    const contact = await Vendor.findById(id);
    const list = await PO.find({ "vendor.name": contact.name }).sort({
        updatedAt: -1,
    });
    if (!contact || !list) {
        return res
            .status(404)
            .json({ message: "failed to fetch vendor details" });
    }
...
export { getVendors, createVendor, getVendor, deleteVendor, updateVendor };
