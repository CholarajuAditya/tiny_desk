...
const ContactList = ({ url, entityName }) => {
    ...
    const createContact = async (e) => {
        ...
        try {
            ...
            window.alert(
                error.response?.data?.message || `Error creating ${entityName}`
            );
        }
    };
        ...
        <>
            ...
            {!loading && (
                <div className="flex flex-col items-center max-w-6xl mx-auto p-6">
                    ...
                    <form
                        className={`mt-6 p-8 bg-gray-50 rounded-lg shadow-md ${
                            createNew ? "block" : "hidden"
                        }`}
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            {Object.keys(newContactAddress).map(
                                (field, index) => (
                                    <label
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-20">
                        {filteredContactList.map((contact, index) => (
                            <NavLink
                                key={index}
                                to={`/user/${entityName}/${contact._id}`}
                                className="block"
                            >
